...
func (t *DBSetup) Run() error {
	...
	if t.Vendor == "" {
		return errors.New("DB_VENDOR is not set, or use WLS_DB_VENDOR alternatively")
	}
	if t.Host == "" {
		return errors.New("DB_HOST is not set, or use WLS_DB_HOSTNAME alternatively")
	}
	if t.Port == 0 {
		return errors.New("DB_PORT is not set, or use WLS_DB_PORT alternatively")
	}
	if t.DBName == "" {
		return errors.New("DB_NAME is not set, or use WLS_DB_NAME alternatively")
	}
	if t.Username == "" {
		return errors.New("DB_USERNAME is not set, or use WLS_DB_USERNAME alternatively")
	}
	if t.Password == "" {
		return errors.New("DB_PASSWORD is not set, or use WLS_DB_PASSWORD alternatively")
	}
	if t.SSLMode == "" {
