...
type KeyDetails struct {
	...
	TransferPolicyAttributes *kbs.KeyTransferPolicy
...
func (keyInfo *KeyDetails) SetUserContext(userCommonName string, cfg *config.Configuration, caCertDir string) error {
	...
	jwtcl := aasClient.NewJWTClient(cfg.AASBaseUrl)
...
func (keyInfo KeyDetails) doesCertIssuerCNMatchKeyTransferPolicy() bool {
	...
	commonNamesList := keyInfo.TransferPolicyAttributes.IssuerName
	if len(commonNamesList) == 0 {
		defaultLog.Debug("keytransfer/skc_key_transfer:doesCertIssuerCNMatchKeyTransferPolicy() Issuer common name in key transfer policy is empty")
		return true
	}
...
func (keyInfo *KeyDetails) doesAttestTypeMatchKeyTransferPolicy() bool {
	...
	attestationType := keyInfo.TransferPolicyAttributes.AttestationType
	...
	if len(keyInfo.FinalStmLabels) > 1 {
		for _, stmLabel := range keyInfo.FinalStmLabels {
			for _, attestType := range attestationType {
				if string(stmLabel) == string(attestType) {
					stmLabels = append(stmLabels, string(attestType))
				}
			}
		}
		if len(stmLabels) == 0 {
		...
		for _, attType := range attestationType {
			if string(attType) == keyInfo.FinalStmLabels[0] {
	...
	if sessionFound {
		...
		if keyInfo.ClientCertSHA == keyTransferSession.ClientCertHash {
			if keyInfo.ActiveStmLabel == constants.DefaultSGXLabel {
				...
				if *keyInfo.TransferPolicyAttributes.SGX.Attributes.EnforceTCBUptoDate && attributes.TCBLevel == constants.TCBLevelOutOfDate {
...
func (keyInfo KeyDetails) validateSgxEnclaveIssuer(stmSgxEnclaveIssuer string) bool {
	...
	for _, enclaveIssue := range keyInfo.TransferPolicyAttributes.SGX.Attributes.MrSigner {
...
func (keyInfo KeyDetails) validateSgxEnclaveIssuerProdId(stmSgxEnclaveIssuerProdID string) bool {
	...
	for _, productId := range keyInfo.TransferPolicyAttributes.SGX.Attributes.IsvProductId {
		if uint16(sgxEnclaveIssuerProdID) == productId {
...
func (keyInfo KeyDetails) validateSgxEnclaveMeasurement(stmSgxEnclaveMeasurement string) bool {
	...
	if stmSgxEnclaveMeasurement == "" && len(keyInfo.TransferPolicyAttributes.SGX.Attributes.MrEnclave) == 0 {
	...
	for _, sgxEnclaveMeasurement := range keyInfo.TransferPolicyAttributes.SGX.Attributes.MrEnclave {
...
func (keyInfo KeyDetails) validateSgxIsvSvn(stmSgxIsvSvn string) bool {
	...
	if uint16(sgxSvn) == *keyInfo.TransferPolicyAttributes.SGX.Attributes.IsvSvn {
