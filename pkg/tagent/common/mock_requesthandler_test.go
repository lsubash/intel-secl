...
func TestMockRequestHandlerImpl_ProvisionImaFiles(t *testing.T) {
	var tagValue = config.TpmConfig{TagSecretKey: tpmSecretKey}
	type fields struct {
		cfg *config.TrustAgentConfiguration
	}
	...
	tests := []struct {
		...
		{
			name: "ProvisionImaFiles by mocking",
			fields: fields{
				cfg: &config.TrustAgentConfiguration{
					Tpm: tagValue,
				},
			...
			args: args{
				reprovisionFilePath: "../test/Resource",
				provisionRequest:    &taModel.ReprovisionImaRequest{Files: []string{"file1", "file2"}},
			},
			wantErr: false,
		},
		{
			name: "Httptest mode failed for ProvisionImaFiles",
			fields: fields{
				cfg: &config.TrustAgentConfiguration{
					Tpm:  tagValue,
					Mode: "httptest",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mrh := &MockRequestHandlerImpl{
				cfg: tt.fields.cfg,
			}
			if err := mrh.ProvisionImaFiles(tt.args.reprovisionFilePath, tt.args.provisionRequest); (err != nil) != tt.wantErr {
				t.Errorf("MockRequestHandlerImpl.ProvisionImaFiles() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
